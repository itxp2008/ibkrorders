        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="md:grid md:grid-cols-3 md:gap-6">
                ...
                <div class="mt-5 md:mt-0 md:col-span-2">
                    <form action="{{ route('ib.postOrder') }}" method="POST">
                        @csrf
                        <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-md">
                            <div class="grid grid-cols-6 gap-6">
                                <!-- acctId -->
                                <div class="col-span-6 sm:col-span-4">
                                    <x-jet-label for="acctId" value="{{ __('acctId') }}" />
                                    <select id="acctId" name="acctId" class="mt-1 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                                        @foreach ($accounts as $acctId => $selected)
                                            <option value="{{ $acctId }}" @if(old('acctId')==$acctId) selected @elseif($selected) selected @endif>{{ $acctId }}</option>
                                        @endforeach
                                    </select>
                                    <x-jet-input-error for="acctId" class="mt-2" />
                                </div>
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    @enderror
                                    <input name="conid" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" id="conid" type="text" value="{{ old('conid') }}">
                                </div>
